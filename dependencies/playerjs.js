...
//	Decode by irfanykywz, script from ???
...
function Playerjs(options) {
	var o = {
		...
		u: {
			...
			control_next: {
				...
				on: 0,
				icon:
				"<svg width='20' height='20'><g transform='translate(5, 5)'><path d='M0.46948,1e-05 L0.46948,1e-05 L0.46948,0 L7.46947,5.21875 L0.46948,10.4375 L0.46948,1e-05 Z M7.53052,0 L9.53052,0 L9.53052,10.62482 L7.53052,10.62482 L7.53052,0 Z' fill='#ffffff'/></g></svg>",
				action: "next",
				type: "svg",
				scale: 1.2,
				position: "controls",
			...
			control_share: {
				...
				on: 0,
				icon:
				"<svg width='20' height='20'><g fill-rule='nonzero' fill='#000000' transform='translate(1, 4)'><path d='M6.62154532,10.2112941 L0.46728972,6.86186551 L0.46728972,5.39643697 L6.62154532,2.03329412 L6.62154532,3.92929411 L2.317637,6.15157982 L6.62154532,8.31386551 L6.62154532,10.2112941 Z M11.289349,0.235294118 L8.67788136,12.2352941 L7.15400168,12.2352941 L9.76546929,0.235294118 L11.289349,0.235294118 Z M16.1019457,6.15157982 L11.797491,3.92957982 L11.797491,2.03357983 L17.9517465,5.39643697 L17.9517465,6.86186551 L11.797491,10.2112941 L11.797491,8.31386551 L16.1019457,6.15157982 Z' fill='#ffffff'/></g></svg>",
				action: "share",
				type: "svg",
				position: "top-right",
				margin: "15 15 0 0",
			...
			rightmenu: 0,
			rc_custom: 1,
			rc_label: "decodeby-irfanykywz",
			rc_labelurl: "https://irfanykywz.blogspot.com",
			rc_version: 1,
			rm1: 0,
			rm1a: "",
			rm1t: "Download",
			...
			control_ILWhz: {
				...
				action: "custom",
				icon:
				"<svg width='20' height='20'><g fill-rule='nonzero' fill='#ffffff' transform='translate(3, 3)'><path d='M0,13 L12,13 L12,10 L14,10 L14,13 L14,15 L0,15 L0,13 Z M0,10 L2,10 L2,13 L0,13 L0,10 Z M5,0 L9,0 L9,7 L11,7 L7,12 L3,7 L5,7 L5,0 Z' fill='#ffffff'></path></g></svg>",
				type: "svg",
				position: "controls",
				margin: "0 0 0 15",
				link: 1,
				...
				aover: "-1",
				tip: 1,
				tiptext: "Download this video",
				mobileover: 0,
			},
			settings: {
				settings4: 0,
				settings4hide: 0,
				settings4action: "share",
				settings5: 1,
				settings6: 1,
				settings7: 0,
			...
			intros: 0,
			introskiptime: "1",
			intro: "https://stream.[web].com/video/[video].mp4",
			vast_introtimebreak: "5",
			vast_introtimelimit: "-1",
			chromecast: {
				on: 0,
			},
			effects: 1,
			...
			control_c0fAo: {
				on: 0,
				text: "Video still processing, check back in 5 minutes",
				order: 20,
				action: "custom",
				type: "text",
				position: "top",
				...
				hidewovar: 0,
				hidevar2: "file",
				hidevar: "hide_processing",
			},
			alert404: 0,
			alert404text: "test alert 3",
			alert404v: 0,
			emptyremove: 1,
			alertsfontsize: 10,
			alerts: 1,
    ...
    var Settings = function (is) {
    	...
    	"' height='" +
    	(style.activeiconsize * 2 > style.valuefontsize ? style.activeiconsize * 2 : style.valuefontsize) +
    	"' xmlns:xlink='http://www.w3.org/1999/xlink' xmlns='http://www.w3.org/2000/svg'><g><ellipse ry='" +
    	style.activeiconsize +
    	"' rx='" +
    	style.activeiconsize +
    	"' cy='" +
    	(style.activeiconsize * 2 > style.valuefontsize ? style.activeiconsize : style.valuefontsize / 2) +
    	"' cx='" +
    	style.activeiconsize +
    	"' fill='#" +
    	style.valuecolor +
    	...
    	function Action(i, cstm, fa) {
    		...
    		if (exist(faction[i])) {
    			...
    			if (open_action != faction[i]) {
    				...
    				var suboptns = false;
    				f2title[0].innerHTML =
    				(x[0] < 2 || _cstm == 1 || style.noprevicon == 1 ? "" : _prevIcon) +
    				(v.settings[is + i + "title"] && v.settings[is + i + "title"] != "" ? v.settings[is + i + "title"] : Lang(v.settings[is + i + "action"])) +
    							...
    							if (typeof z[j] == "string") {
    								if (z[j].indexOf("pjslng") == 0) {
    									...
    									if (t.indexOf("color") > 0) {
    										f2value[y].innerHTML =
    										"<div style='" +
    										(v[t] == "000000" ? "border:1px solid #999;height:8px;width:8px;" : "height:10px;width:10px;") +
    										";background-color:" +
    										(v[t].indexOf("#") == -1 ? "#" : "") +
    										v[t] +
    	...
    	for (y = 1; y <= values.length; y++) {
    		...
    		if (x.indexOf("color") > 0 || x.indexOf("bottom") > 0 || x.indexOf("timer") > 0 || x.indexOf("shift") > 0) {
    			...
    			if (x.indexOf("color") > 0) {
    				...
    				f2title[y].innerHTML =
    				"<div style='" + (values[y - 1] == "000000" ? "border:1px solid #999;height:18px;width:18px;" : "height:20px;width:20px;") + ";background-color:" + CheckColor(values[y - 1]) + ";border-radius:20px;'></div>";
    				css(f2[y], {
    					"line-height": 1,
    				});
    			} else {
    ...
    function StyleArrow(x, y, z, x1, y1, x2, y2, x3, y3, x4, y4) {
    	...
    	(style.scrollarrowbgover == 1
    		...
    		"\"' onMouseOut='" +
    		(style.scrollarrowbg == 1 ? 'this.style.backgroundColor="#' + style.scrollarrowbgcolor : 'this.style.background="none') +
    		"\"'"
    		: "") +
...
var PluginShare = function (url) {
	...
	this.Show = function () {
		if (!created) {
			...
			(exist(v.url)
				? '<a href="' + v.url + '" style="text-decoration:none;color:' + clr + ';outline:0;white-space: nowrap;letter-spacing:1px;display:block" target="_blank" id="pjs_share_link_' + v.id + '">' + v.url + "</a>"
				: "");
	...
	function ClickAction(y, z) {
		...
		if (y == "embed" && exist(v.embed)) {
			...
			copy(
				(exist(v.embedhtmlstart) ? v.embedhtmlstart.replace(/pjs'qt/gi, '"') : "") +
				'<iframe width="' +
				...
				"></iframe>" +
				(exist(v.embedhtmlend) ? v.embedhtmlend.replace(/pjs'qt/gi, '"') : "")
				);
			if (z) {
				o.alert.txt(Lang("copied"), 2);
			} else {
...
function PluginShare_webshare() {
	this.share = function () {
		if (navigator.canShare) {
			try {
				var x = {
					url: exist(v.url) ? v.url : o.href,
				};
				if (exist2(v.websharetext)) {
					x.text = v.websharetext;
				}
				navigator.share(x);
			} catch (e) {
				console.log(e);
...
var PluginWater = function () {
	var x;
	var z = false;
	o.waterInterval = setInterval(Start, (exist(v.waterpause) ? v.waterpause : 5) * 60 * 1000);
	if (v.wid_test == 1) {
		setTimeout(Start, 5);
	}
	...
	function Start() {
		if (o.play) {
			if (z) {
				o.mediacontainer.removeChild(x);
			}
			x = createElement("div");
			var w = random(0, o.screen_w);
			var h = random(0, o.screen_h);
			css(x, {
				position: "absolute",
				top: 0,
				left: 0,
				color: exist(v.watercolor) ? v.watercolor : "#fff",
				"font-size": exist(v.waterfont) ? v.waterfont : 8,
				opacity: v.wid_test == 1 ? 1 : exist(v.wateropacity) ? v.wateropacity : 0.1,
			});
			if (w > o.screen_w / 2) {
				css(x, {
					left: "auto",
			...
			o.mediacontainer.appendChild(x);
			z = true;
			setTimeout(Stop, (exist(v.watertime) ? v.watertime : 2) * 1000);
			x.innerHTML = v.wid;
		}
	}
				...
				if (exist(sub[x])) {
					if (exist(sub[x][1])) {
						if (exist(sub[x][1][t])) {
							...
							'<span style="' +
							(v.sub_bg == 1 ? "background-color:" + hex2rgb(v.sub_bgcolor, v.sub_bga) + ";" : "") +
...
var YoutubeID = function (url) {
	...
	var CustomFonts = function () {
		if (v.fonts == 1 && exist(v.fontnames) && v.fontnames != "") {
			...
			x.rel = "stylesheet";
			x.href = "https://fonts.googleapis.com/css?family=" + v.fontnames.replace(/,/gi, "|").replace(/ /gi, "+");
			...
			if (pip) {
				...
				if (pjstg.audioTracks && o.files_audiotrack.length == 0 && v.nativeaudio != 0) {
					...
					for (i = 0; i < at.length; i++) {
						var x = at[i].label + " (" + at[i].language + ")";
						if (typeof v.rename_audio == "object") {
							exist(v.rename_audio[x]) ? (x = v.rename_audio[x]) : exist(v.rename_audio[i.toString()]) ? (x = v.rename_audio[i.toString()]) : "";
						}
	...
	var ControlLine = function (key, action) {
		...
		StyleLine(
			...
			line1,
			style.linespeed1,
			style.gradient == 1 ? "linear-gradient(" + (style.grdlft == 1 ? "to left," : "") + CheckColor(style.gradientcolorload) + ", " + CheckColor(style.colorload) + ")" : style.colorload,
			0,
			style.aload
			);
